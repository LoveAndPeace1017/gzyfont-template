...
router.get('/list', function(req, res, next) {
    ...
    let url = `/pc/v1/${session.userIdEnc}/saleorders/list`;
    if(params.source === 'mall'){
        url = `/pc/v1/${session.userIdEnc}/saleorders/online/list`;
    }
    ...
    backend.post(url, params, req, res, function(data) {
        if (data && data.retCode == 0) {
            ...
            list = list.map((item, index) => {
                ...
                item.serial = index+1;
                item.actualOutAmount = item.outAmount - item.returnAmount;
...
router.post('/delete', function(req, res) {
    ...
    const session = Session.get(req, res);
    // const userId = 'nEoAUdKFbkYs';
    backend.delete(`/pc/v1/${session.userIdEnc}/saleorders?isCascaded=0`, params, req, res, function(data) {
        res.json(data);
    });
});
...
router.get('/listSale', function (req, res) {
    ...
    params.saleBillNo = req.query.billNo;
    backend.get(`/pc/v1/${session.userIdEnc}/saleorders/listSale`, params, req, res, function (data) {
        res.json(data)
    });
});
...
router.get('/pre/create', function(req, res, next) {
    const params = req.query;
    const session = Session.get(req, res);
    backend.get(`/pc/v1/${session.userIdEnc}/saleorders/pre/create`, params, req, res, function(data) {
        if (data && data.retCode == 0) {
            data.tags = PropertyFilter.initCustomTags(data.tags);
...
router.post('/add', function(req, res, next) {
    ...
    const session = Session.get(req, res);
    backend.post(`/pc/v1/${session.userIdEnc}/saleorders`, params, req, res, function(data) {
        res.json(data);
    });
});
/* 修改销售 */
router.post('/modify/:billNo', function(req, res, next) {
    ...
    const session = Session.get(req, res);
    backend.put(`/pc/v1/${session.userIdEnc}/saleorders/${req.params.billNo}`, params, req, res, function(data) {
        res.json(data);
    });
});
...
router.get('/detail/:id', function(req, res) {
    const session = Session.get(req, res);
    backend.get(`/pc/v1/${session.userIdEnc}/saleorders/${req.params.id}`, req, res, function(data) {
        if (data && data.retCode == 0) {
            data.tags = PropertyFilter.initCustomTags(data.tags);
            data.prodDataTags = PropertyFilter.initCustomTags(data.prodDataTags, 'prod_');
            data.billProdDataTags = PropertyFilter.initCustomTags(data.billProdDataTags, 'item_');
            ...
            if(data.data && data.data.prodList){
                data.data.prodList = PropertyFilter.dealProdCustomField({list: data.data.prodList, prefix: 'item_', propertyKey: 'propertyValues'});
