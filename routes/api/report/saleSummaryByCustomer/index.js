...
const map = {
    saleOrderDate: {
        label: "node.report.saleSummaryByCustomer.saleOrderDate",
        width: Constants.TABLE_COL_WIDTH.DATE
    },
    customerName: {
        label: "node.report.saleSummaryByCustomer.customerName"
    },
    contacterName: {
        label: "node.report.saleSummaryByCustomer.contacterName"
    },
    telNo: {
        label: "node.report.saleSummaryByCustomer.telNo"
    },
    email: {
        label: "node.report.saleSummaryByCustomer.email"
    },
    levelName: {
        label: "node.report.saleSummaryByCustomer.levelName"
    },
    legalRepresentative: {
        label: "node.report.saleSummaryByCustomer.legalRepresentative"
    },
    registeredAddress: {
        label: "node.report.saleSummaryByCustomer.registeredAddress"
    },
    licenseNo: {
        label: "node.report.saleSummaryByCustomer.licenseNo"
    },
    customerCount: {
        label: "node.report.saleSummaryByCustomer.customerCount",
        totalFlag: true
    },
    amount: {
        label: "node.report.saleSummaryByCustomer.amount",
        columnType: 'money',
        totalFlag: true
    },
    payAmount: {
        label: "node.report.saleSummaryByCustomer.payAmount",
        columnType: 'money',
        totalFlag: true
    },
    waitPay: {
        label: "node.report.saleSummaryByCustomer.waitPay",
        columnType: 'money',
        totalFlag: true
    }
};
...
router.post('/export', function (req, res, next) {
    ...
    backend.post(`/pc/v1/${session.userIdEnc}/report/saleorder/summary/by/customer`, params, req, res, function (data) {
        if (data && data.retCode == 0) {
            ...
            list.forEach(function (item) {
                item.key = i;
                item.serial = i++;
                item.waitPay = item.amount-(item.payAmount || 0);
            });
