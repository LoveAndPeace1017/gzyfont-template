...
// 工序列表
const processList = withBatchUpdateReducer(constant.TYPE, (
    state = fromJS({
        isFetching: false,
        data: []
    }),
    ...
    switch (action.type) {
        case constant.FETCH_PROCESS_LIST_CONFIG_SUCCESS:
            return state.set('config',action.data);
        case constant.FETCH_PROCESS_LIST_REQUEST:
            return state.set('isFetching', true);
        case constant.FETCH_PROCESS_LIST_SUCCESS:
            return state.set('isFetching', false)
                .set('data', action.data);
        case constant.FETCH_PROCESS_LIST_FAILURE:
            return state.set('isFetching', false);
