...
const inviteFriend = withAsyncReducer(constant.FETCH_INVITE_FRIEND);
...
const notice = withAsyncReducer(constant.FETCH_NOTICE);
...
const pendingApproval = (
    ...
    switch (action.type) {
    case constant.FETCH_PENDING_APPROVAL_REQUEST:
        return state.set('isFetching', !action.isBackLoading);
    case constant.FETCH_PENDING_APPROVAL_SUCCESS:
        return state.set('isFetching', false)
            .set('data', action.data);
    case constant.FETCH_PENDING_APPROVAL_FAILURE:
        return state.set('isFetching', false);
    default:
        return state
    }
...
// 获取当前模块的单据数量是否达到上限
const checkModuleArriveUpperLimit = withAsyncReducer(constant.FETCH_CHECK_MODULE_HAS_ARRIVE_UPPER_LIMIT);
