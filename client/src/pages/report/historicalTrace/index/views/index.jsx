...
import {
    asyncFetchHistoricalTraceDetailReport,
    asyncUpdateConfig,
...
export class Index extends ListPage {
    constructor(props) {
        ...
        const batchNo = searchQuery && searchQuery.query.batchNo;
        const serialNumber = searchQuery && searchQuery.query.serialNumber;
    ...
    componentDidMount() {
        if (this.serialNumber || this.prodNo || this.batchNo) {
    ...
    batchUpdateConfig = (callback) => {
        ...
        let moduleType = {
            table:'historicalRetrospection_list'
        };
    ...
    refreshReportForm = () => {
        ...
        console.log(param);
        this.props.asyncFetchHistoricalTraceDetailReport(param || {}, (data) => {
            if (data.retCode != 0) {
    ...
    generateReportForm = () => {
        this.batchUpdateConfig(() => {
            ...
            console.log(param);
            if((!param.prodNo) || (!param.batchNo && !param.serialNumber)){
                message.error('请至少选择一个物品，并输入查询的序列号或批次号')
            }else{
                this.props.asyncFetchHistoricalTraceDetailReport(param || {}, (data) => {
                    if (data.retCode != 0) {
                        message.error(data.retMsg);
    ...
    render() {
        ...
        const filterConfigList = [{
            label: 'report.historicalTrace.prodNo',
            fieldName: 'prodNo',
            visibleFlag: true,
            displayFlag: true,
            type: 'product',
        ...
        tableConfigList.forEach((item) => {
            if (item.visibleFlag && item.label) {
                ...
                if(item.fieldName === "date"){
                    obj.render = (date) => {
                        return date?(<span>{moment(date).format('YYYY-MM-DD')}</span>):null
                    };
                }
        ...
        const filterDataSource = {
            inputNoBtnComponents:[
                {
                    label: "report.historicalTrace.batchNo",
                    fieldName: 'batchNo',
                    defaultValue: this.batchNo
                },
                {
                    label: "report.historicalTrace.serialNumber",
                ...
                <div className="content-index-bd">
                    <div className="list-ope-wrap">
                        <ListOpeBar
                            ...
                            exportType="historicalTraceReport"
                            onReset={()=> {
                    ...
                    <div className={"data-wrap cf"}>
                        <div className={"tb-wrap"}>
                            <ReportHd
                                ...
                                sourceName={"系统内各物品的出入库记录和生产记录数据"}
                            />
                            <ListTable
                                className={"report"}
                                columns={tempColumns}
...
const mapStateToProps = (state) => ({
    reportDetail: state.getIn(['historicalTraceReportIndex', 'historicalTraceDetail'])
});
