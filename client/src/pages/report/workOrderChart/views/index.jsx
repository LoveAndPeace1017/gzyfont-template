...
import {
    asyncFetchWorkChartDetailReport
} from '../actions';
...
export class Index extends ListPage {
    ...
    generateReportForm = () => {
        ...
        if(param.startTime){
            console.log(param);
            this.props.asyncFetchWorkChartDetailReport(param || {}, (data) => {
                if (data.retCode != 0) {
    ...
    render() {
        ...
        const filterConfigList = [{
            ...
            options: [
                ...
                {label: intl.get("report.sale.option2"), value: '1'},
        ...
        for(let i=0;i<dataSource.length;i++){
            ...
            dataSource1.push({
                key: i+1,
                name: <div style={{padding: 14+44.165*formatecPwspList+"px 20px"}} className={cx("y-dom")}>
                    <ul>
                        <li title={dataSource[i].caName}>
                            工作中心：{dataSource[i].caName}
                        </li>
                                    ...
                                    lastDataSource.map((item,index)=>{
                                        if(item.actualFlag){
                                            return <React.Fragment>
                                                <NewProgress key={index} data={item} actualFlag={false} status={item.processStatus} width={item.expectWidth} index={index+1} top={item.expectTop} left={item.expectLeft}/>
                                                <NewProgress key={index+'ls'} data={item} actualFlag={true} status={item.processStatus} width={item.actualWidth} index={index+1} top={item.actualTop} left={item.actualLeft}/>
                                            </React.Fragment>
                                        }else{
                                            return <NewProgress key={index} data={item} actualFlag={false} status={item.processStatus} width={item.expectWidth} index={index+1} top={item.expectTop} left={item.expectLeft}/>
                                        }
                                    })
                                }
                            </div>
...
const mapStateToProps = (state) => ({
    reportDetail: state.getIn(['auxiliaryWorkChart', 'workChartDetail'])
});
