...
export default class Index extends Component {
    ...
    onPageInputChange = (page, perPage) => {
        this.props.fetchData({activeKey: 'gainMaterialRecord', params: {perPage, page}});
    };
    onShowSizeChange = (current, perPage) => {
        this.props.fetchData({activeKey: 'gainMaterialRecord', params: {perPage, page: 1}});
    };
        ...
        let columns = [
            ...
            },
            {
                ...
                render: (displayBillNo, record) => (
                    <span className={cx("txt-clip")} title={displayBillNo}>
                          <Link ga-data="list-billNo" to={`/inventory/outbound/show/${record.billNo}`}>{displayBillNo}</Link>
                    </span>
                )
            },
            {
                ...
                render: (outDate)=>{
                    return (<span>{outDate ? moment(outDate).format('YYYY-MM-DD') : ''}</span>)
                }
            },
            {
                title: "仓库",
            ...
            },
            {
                ...
                render: (prodAbstract, data) => (
                    <Dropdown
                        onVisibleChange={(visible) => this.props.loadWare(visible, data.billNo, 'gainMaterialRecord')}
                        overlay={() => (
