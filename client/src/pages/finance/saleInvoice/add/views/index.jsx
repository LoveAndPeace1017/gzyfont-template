...
import { Row, Col, Input, Radio, message, DatePicker, Spin, Modal } from 'antd';
...
import {asyncAddSaleInvoice, asyncEditSaleInvoice, asyncFetchSaleInvoiceById} from '../actions';
...
const mapStateToProps = (state) => ({
    ...
    addSaleInvoice: state.getIn(['saleInvoiceAdd', 'addSaleInvoice']),
});

const mapDispatchToProps = dispatch => {
    return bindActionCreators({
        asyncAddSaleInvoice,
        asyncEditSaleInvoice,
...
@withApprove
@oldFormCreate
export default class SaleInvoiceAddForm extends addPage {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        if (recId) {
            ...
            this.props.asyncFetchSaleInvoiceById(recId, (data) => {
                ...
                this.props.form.setFieldsValue({
                    ...
                    customerName: formData.getIn([0, 'customerName']),
    ...
    handleEditResult = (data) => {
        ...
        if (data && data.retCode == 0) {
            message.success(intl.get("saleInvoice.add.index.operateSuccessMessage"));
            this.props.emptyFieldChange();
            ...
            let url = continueAddFlag ? '/blank' :`/finance/saleInvoice/show/${id}`;
            this.props.history.push(url);
            continueAddFlag && this.props.history.goBack();
        }
        else {
            Modal.error({
                title: intl.get("saleInvoice.add.index.warningTip"),
                content: (data && data.retMsg) || intl.get("saleInvoice.add.index.operateFailureMessage")
            });
        ...
        this.props.form.validateFields((err, fieldsValues) => {
            ...
            if (!err||(fieldsValues.bindBillType == 0 && (removeEmptyAry[0].displayBillNo!=undefined || removeEmptyAry[0].amount!=undefined))) {
                ...
                this.setState({submitData: values},()=>{
                    this.props.asyncGetApproveStatus({types: BACKEND_TYPES.saleInvoice}, (res) => {
    ...
    asyncSubmit=()=>{
        ...
        if (this.state.option === EditType.Add || this.state.option === EditType.Copy) {
            this.props.asyncAddSaleInvoice(values, function(data) {
                self.handleEditResult(data);
            });
        }
        else {
            this.props.asyncEditSaleInvoice(values, function(data) {
                self.handleEditResult(data);
            });
        }
    };
    ...
    render() {
        ...
        const dataList = saleInvoiceInfo.getIn(['data', 'data']);
        ...
        return (
            <React.Fragment>
                    ...
                    <Crumb data={[
                        {
                            ...
                            title: intl.get("saleInvoice.add.index.saleInvoice")
                        },
                        {
                            title: this.state.option === EditType.Modify ? intl.get("saleInvoice.add.index.modifySaleInvoiceRecord") : intl.get("saleInvoice.add.index.addSaleInvoiceRecord")
                        }
                    ]}/>
                </div>
                {/*<SelectBill visible={this.state.selectModalVisible}
                            ...
                            selectedKeys={[]}/>*/}
                <Spin
                    spinning={saleInvoiceInfo.get("isFetching")}
                >
                    <AddForm onSubmit={this.handleSubmit}
                             ...
                             loading={this.props.addSaleInvoice.get('isFetching')}>
                        <div className="content-bd">
                            <Fold title={intl.get("saleInvoice.add.index.baseInfo")}>
                                <Row>
                                    <Col span={8}>
                                        <Form.Item label={intl.get("saleInvoice.add.index.invoiceDate")} required={true} {...formItemLayout}>
                                            {
                                                getFieldDecorator("invoiceDate", {
                                                    initialValue: moment(),
                                                    rules: [{type: 'object', required: true, message: intl.get("saleInvoice.add.index.invoiceDateMessage")}]
                                                })(
                                                    <DatePicker/>
                                                )
                                            }
                                    ...
                                    <Col span={8}>
                                        <Form.Item label={intl.get("saleInvoice.add.index.customerName")}  {...formItemLayout}>
                                            {getFieldDecorator('customerName', {
                                                ...defaultOptions,
                                            })(
                                                <Input maxLength={80} disabled={!this.state.customerNameEditEnabled}
                                                       placeholder={intl.get("saleInvoice.add.index.customerNamePlaceholder")}/>
                                            )}
                                        </Form.Item>
                                    </Col>
                                    <Col span={8}>
                                        <Form.Item label={intl.get("saleInvoice.add.index.invoiceCustomNo")}  {...formItemLayout}>
                                            {getFieldDecorator('invoiceCustomNo', {
                                                ...defaultOptions,
                                            })(
                                                <Input placeholder={intl.get("saleInvoice.add.index.invoiceCustomNoPlaceholder")}/>
                                            )}
                                        </Form.Item>
                                    </Col>
                                    <Col span={8}>
                                        <Form.Item label={intl.get("saleInvoice.add.index.invoiceType")} {...formItemLayout}>
                                    ...
                                    <Col span={8}>
                                        <Form.Item label={<React.Fragment/>} colon={false}  {...formItemLayout}>
                                            {getFieldDecorator('bindBillType', {
                                                ...
                                                <RadioGroup>
                                                    <Radio key={1} value={1}>{intl.get("saleInvoice.add.index.bindBillType_1")}</Radio>
                                                    <Radio key={0} value={0}>{intl.get("saleInvoice.add.index.bindBillType_0")}</Radio>
                                                </RadioGroup>
                                            )}
                                        </Form.Item>
                                    </Col>
                                    {
                                            <Col span={8} style={getFieldValue('bindBillType') === 0?{display:"block"}:{display:"none"}}>
                                                <Form.Item label={intl.get("saleInvoice.add.index.amount")} {...formItemLayout}>
                                                    {getFieldDecorator('amount', {
                                                        ...defaultOptions,
                                                        rules: getFieldValue('bindBillType') === 0?[{
                                                            required: true,
                                                            message: intl.get("saleInvoice.add.index.amountMessage1")
                                                        }, {
                                                            validator: (rules, value, callback) => {
                                                                const reg = /^(0|[1-9]\d{0,9})(\.\d{1,2})?$/;
                                                                if (Number.isNaN(value) || !reg.test(value)) {
                                                                    callback(intl.get("saleInvoice.add.index.amountMessage2"))
                                                                }
                                                                if (value == 0) {
                                                                    callback(intl.get("saleInvoice.add.index.amountMessage3"))
                                                                }
                                                                callback();
                                                            }
                                                        }]:[],
                                                    })(
                                                        <Input placeholder={intl.get("saleInvoice.add.index.amountPlaceholder")} autoFocus={true} maxLength={13}
                                                               suffix={intl.get("saleInvoice.add.index.yuan")}/>
                                                    )}
                                                </Form.Item>
                                            </Col>
                                    }
                                </Row>
                                {
                                        ...
                                        <div style={getFieldValue('bindBillType') === 1?{display:"block"}:{display:"none"}}>
                                        <AddForm.ProdList>
                                            <UpperBill
                                                {...this.props}
                            ...
                            <Fold title={intl.get("saleInvoice.add.index.otherInfo")}>
                                <Row>
                                    <Col span={24}>
                                        <Form.Item
                                            label={"经办人"}
                                ...
                                <Row>
                                    <Col span={24}>
                                        <Form.Item
                                            label={intl.get("saleInvoice.add.index.remarks")}
                                            {...otherFormItemLayout}>
                                            {
                                                getFieldDecorator("remarks", {
                                                    ...
                                                    <TextArea rows={4} placeholder={intl.get("saleInvoice.add.index.remarks")} maxLength={2000}/>
                                                )
                                            }
                                        </Form.Item>
                                    </Col>
                                ...
                                <Row>
                                    <Col span={8}>
                                        <Form.Item
                                            label={intl.get("saleInvoice.add.index.tempAtt")}
                                            {...formItemLayout}
                                        >
                                            {
