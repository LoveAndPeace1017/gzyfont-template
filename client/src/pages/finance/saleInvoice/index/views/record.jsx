...
import {asyncFetchSaleInvoiceRecord} from '../actions'
...
export class Record extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.type === 'customer') {
            recordColumns = [
                {
                    title: intl.get("saleInvoice.index.record.serial"),
                    width: 50,
                    dataIndex: 'serial',
                ...
                /*{
                    ...
                    render: (text, record) => {
                        return (
                            <Link to={`/sale/show/${record.saleBillNo}`}>{text}</Link>
                        )
                    }
                },
                {
                ...
                {
                    title: intl.get("saleInvoice.index.record.billNo"),
                    width: 200,
                    dataIndex: 'billNo',
                    render: (text, record) => {
                        return (
                            <Link to={`/finance/saleInvoice/show/${record.id}`}>{text}</Link>
                        )
                    }
                },
                {
                    title: intl.get("saleInvoice.index.record.invoiceNo"),
                    width: 200,
                    dataIndex: 'invoiceNo',
                },
                {
                    title: intl.get("saleInvoice.index.record.invoiceDate"),
                    width: 200,
                ...
                {
                    title: intl.get("saleInvoice.index.record.amount"),
                    width: 150,
                    dataIndex: 'amount',
                ...
                {
                    title: intl.get("saleInvoice.index.record.remarks"),
                    dataIndex: 'remarks'
                }
            ];
            ...
            recordColumns = [
                ...
                {
                    title: intl.get("saleInvoice.index.record.customerName"),
                    width: 200,
    ...
    fetchData = ({page, perPage}) => {
        this.props.asyncFetchSaleInvoiceRecord({type: this.props.type, recordFor: this.props.recordFor, page, perPage});
    };
...
const mapStateToProps = (state) => ({
    recordList: state.getIn(['saleInvoiceIndex', 'saleInvoiceRecord']),
});

const mapDispatchToProps = dispatch => {
    return bindActionCreators({
        asyncFetchSaleInvoiceRecord
    }, dispatch)
};
