...
const onlineOrderCartList = (
    ...
    switch (action.type) {
        ...
        case constant.FETCH_ONLINE_ORDER_CART_CLICK_All_LIST:
            let chooseAllFlag = state.getIn(['validProdList', action.comIndex, 'chooseAllFlag']);

            if(chooseAllFlag !== 2){
              return state.setIn(['validProdList', action.comIndex, 'chooseAllFlag'], 2)
                            .updateIn(['validProdList', action.comIndex, 'items'], function (item) {
                                return item.map((itemList) => itemList.set('flag', true));
                            })
            } else {
                return state.setIn(['validProdList', action.comIndex, 'chooseAllFlag'], 1)
                            .updateIn(['validProdList', action.comIndex, 'items'], function (item) {
