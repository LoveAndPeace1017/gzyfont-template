    ...
    render() {
        ...
        return (
            <div className={cx(['modal-container', {'modal-size': !!showEdit}])}>
                                ...
                                if(this.props.showVisible){
                                    if(item.get('visibleflag') === 0){
                                        return <Option key={item.get('id')}
                                                       value={item.get('id')}>{item.get('projectName')}</Option>
                                    }
                                }else{
                                    return <Option key={item.get('id')}
                                                   value={item.get('id')}>{item.get('projectName')}</Option>
                ...
                <ProjectAdd onClose={this.closeModal.bind(this, 'projectAddVisible')} visible={this.state.projectAddVisible} callback={(projectName, id) => {
