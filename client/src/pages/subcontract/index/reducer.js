...
// 委外加工列表
const subcontractList = withBatchUpdateReducer(constant.TYPE, (
    state = fromJS({
        isFetching: false,
        data: []
    }),
    ...
    switch (action.type) {
        case constant.FETCH_SUBCONTRACT_CONFIG_SUCCESS:
            return state.set('config',action.data);
        case constant.FETCH_SUBCONTRACT_LIST_REQUEST:
            return state.set('isFetching', true);
        case constant.FETCH_SUBCONTRACT_LIST_SUCCESS:
            return state.set('isFetching', false)
                .set('data', action.data);
        case constant.FETCH_SUBCONTRACT_LIST_FAILURE:
            return state.set('isFetching', false);
        case constant.FETCH_OUTSOURCE_PROD_ABSTRACT_BY_BILL_NO_REQUEST:
        ...
        case constant.FETCH_OUTSOURCE_PROD_ABSTRACT_BY_BILL_NO_SUCCESS:
            return state.updateIn(['data', 'list'], list => {
                return list.map(item => {
                    if (item.get('billNo') === action.billNo) {
        ...
        case constant.FETCH_OUTSOURCE_PROD_ABSTRACT_BY_BILL_NO_FAILURE:
            return state.updateIn(['data', 'list'], list => {
                return list.map(item => {
                    if (item.get('billNo') === action.billNo) {
                        return item.set('prodAbstractIsFetching', false)
