...
import {updateProdBindInfo, asyncFetchSaleById, asyncBindProduct, asyncUnbindProduct, asyncFetchOperationLog} from '../../add/actions';
import {asyncDeleteSaleInfo,asyncBeforeDeleteSaleInfo} from '../../index/actions';
...
export default class Index extends detailPage {
    ...
    showConfirm = () => {
        ...
        let self = this;
        //asyncBeforeDeleteSaleInfo
        self.props.asyncBeforeDeleteSaleInfo([id], (res)=> {
            console.log(res, 'res6');
            Modal.confirm({
                title: intl.get("sale.show.index.warningTip"),
                okText: intl.get("sale.show.index.okText"),
                cancelText: intl.get("sale.show.index.cancelText"),
                content: res == 1 ? intl.get("sale.show.index.deleteConfirmContent_1") : intl.get("sale.show.index.deleteConfirmContent_2"),
                okButtonProps:{
                ...
                onOk() {
                    return new Promise((resolve, reject) => {
                        self.props.asyncDeleteSaleInfo([id], function (res) {
                            resolve();
                            if (res.retCode == 0) {
                                message.success(intl.get("sale.show.index.deleteSuccessMessage"));
                                self.props.history.replace('/sale/')
                            } else {
    ...
    saleProductList = () => {
        ...
        const data = prodList.map((item, index) => {
            ...
            prodItem = this.multiPreProcessDataTagValue(billProdDataTags, prodItem); // 处理自定义字段中的特殊值
    ...
    operateBar = () => {
        ...
        listAction = listAction.concat([
            {
                name: 'edit',
                path: '/sale/modify/' + this.getInfo().get('billNo') + `?source=${getUrlParamValue('source')}&current=${getUrlParamValue('current')}`,
                module: 'sale',
            ...
            },
            {
                name: 'copy',
                module: 'sale',
                path: '/sale/copy/' + this.getInfo().get('billNo') + `?source=${getUrlParamValue('source')}&current=${getUrlParamValue('current')}`
            }
        ]);
        ...
        listAction = listAction.concat([
            {
                ...
                module: 'sale',
                href: `${BASE_URL}/file/download?url=/saleorders/excel/export/${this.getInfo().get('billNo')}`,
        ...
        listAction.push({
            ...
            icon: "icon-assign-subaccount",
            label: "生产进度",
            path: `/productControl?saleBillNo=${billNo}&displaySaleBillNo=${this.getInfo().get('displayBillNo')}`,
        },{
            ...
            onClick: () => {
                this.props.vipTipPop({
                    ...
                    onTryOrOpenCallback: async ()=>{
                        const name = info.get('customerContacterName');
                        const mobile = info.get('customerTelNo');
            ...
            <React.Fragment>
                ...
                <div className={cx("status-btn")}>
                    ...
                    <MessageRecommend
                        ...
                        source={'sale'}
                        billNo={billNo}
                        date={moment(info.get('saleOrderDate')).format('YYYY年MM月DD日')}
                    />
                </div>
            </React.Fragment>
        )
            ...
            <Layout>
                <div>
                    <Crumb data={getUrlParamValue('source') === 'mall'?[
                        ...
                        {url: '/sale', title: intl.get("sale.show.index.saleList")},
                        {title: intl.get("sale.show.index.detail")}
                    ]}/>
                    ...
                    <div className={cx("float-right")}>
                        <IsVipJudge templateType={"SaleOrder"}/>
                    </div>
                </div>
                <div className="detail-content"  style={{position:"relative"}}>
                    ...
                    <div className="detail-content-bd"  style={{position:"relative"}}>
                        ...
                        <PageTurnBtn type={"sale"}  current={billNo}/>
                        <Tabs
                                ...
                                detailData && detailData.get('state') === 0 && (
                                    <>
                                        ...
                                        {this.props.renderFinishButton({
                                            source: 'sale', module: 'outbound', billNo, style: {marginRight: '10px'},
                                            callback: () => {
                                                this.wareOut.fetchData({});
                                                this.loadData();
                                            }
                                        })}
                                        <OutboundRecord
                                ...
                                detailData && detailData.get('state') === 1 && (
                                    <>
                                        {this.props.renderHandOperateRecord({
                                            source: 'sale', module: 'outbound', billNo,
                                            callback: () => {
                                                this.wareOut.fetchData({});
                                                this.loadData();
                                            }
                                        })}
                                        <Fold title={"系统出库记录"} rightContent={()=>{
                                            return (
                                                <Button type={"primary"} style={{float: 'right', marginBottom: '5px'}}>
                                                    <Link to={`/report/inventory/detail?saleBillNo=${displayBillNo}`}>查询明细</Link>
                                                </Button>
                        ...
                        } key="saleInvoiceRecord">
                            {
                                detailData && detailData.get('invoiceState') === 0 && (
                                    <>
                                        {
                                            this.props.renderFinishButton({
                                                source: 'sale', module: 'saleInvoice', billNo,
                                                callback: () => {
                        ...
                        } key="incomeRecord">
                            {
                                detailData && detailData.get('payState') === 0 && (
                                    <>
                                        {
                                            this.props.renderFinishButton({
                                                source: 'sale', module: 'income', billNo,
                                                callback: () => {
                                                    this.incomeRecord.fetchData({});
                                                    this.loadData();
                                                }
                                            })
                                        }
