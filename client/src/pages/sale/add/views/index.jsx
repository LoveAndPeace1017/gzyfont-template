...
import {actions as quotationAction} from 'pages/quotation/add';
...
import {asyncFetchPreData, asyncAddSale, asyncFetchSaleById, emptyDetailData} from '../actions';
...
const mapStateToProps = (state) => ({
    addSale: state.getIn(['saleAdd', 'addSale']),
    saleInfo: state.getIn(['saleAdd', 'saleInfo']),
    preData:  state.getIn(['saleAdd', 'preData']),
...
const mapDispatchToProps = dispatch => {
    return bindActionCreators({
        ...
        asyncFetchSaleById,
        emptyDetailData,
        asyncFetchQuotationById: quotationAction.asyncFetchQuotationById,
...
export default class SaleAddForm extends addPage {
    ...
    preDataProcess = async () => {
        ...
        const quotationCode = params && params.get('quotationCode'); //根据报价单号带入单据信息
        await this.initPreData();
        await this.initRate();
        productCode && await this.initSingleGoods(productCode);
        if(quotationCode) modifyBackData = await this.initQuotationInfo(quotationCode);
        if(id) modifyBackData = await this.initModifyData(id);
        customerNo = modifyBackData.customerNo || customerNo;
        let initFlag = quotationCode?true:!!id;
    ...
    initModifyData = (id) => {
        return new Promise((resolve, reject) => {
            this.props.asyncFetchSaleById(id, data=>{
                let _this = this;
                let info = data.data||{};
                // 初始化修改物品
        ...
        return new Promise((resolve, reject) => {
            this.props.asyncFetchQuotationById(id, (data)=>{
                ...
                let initInfo = {
                    ...
                    customerContacterName: info.customerContacterName,
                    customerTelNo: info.customerTelNo,
                    currencyFlag: info.currencyFlag,
                    currencyId: info.currencyId,
                    currencyName: info.currencyName,
                    quotation: info.quotation,
                    deliveryAddress: info.deliveryAddress,
                    deliveryCityCode: info.deliveryCityCode,
                    deliveryCityText: info.deliveryCityText,
                    deliveryProvinceCode: info.deliveryProvinceCode,
                    deliveryProvinceText: info.deliveryProvinceText,
                    projectName: info.projectName,
                    settlement: info.settlement,
                    customerOrderNo: info.customerOrderNo,
                    ourName: info.ourName,
                    ourContacterName: info.ourContacterName,
                    departmentId: info.departmentId,
                    departmentName: info.departmentName,
                    ourTelNo: info.ourTelNo,
                    remarks: info.remarks,
                    prodList: info.prodList,
                    totalQuantity: info.totalQuantity,
                    taxAllAmount: info.taxAllAmount,
                    discountAmount: info.discountAmount,
                    aggregateAmount: info.aggregateAmount,
                    currencyAggregateAmount: info.currencyAggregateAmount,
                };
                // 初始化修改物品
                let prodList = this.preProcessProd({prodList: info.prodList, billProdDataTags: data.billProdDataTags});
                // 初始化表单数据
                this.initForm(initInfo);
                // 初始化物品表单数据
                this.initProdForm(prodList);
                // 初始化合计
                this.initFormTotal(initInfo);
                resolve({customerNo: initInfo.customerNo});
            })
        })
    }
    ...
    /** 处理物品列表*/
    dealProdList = (values) => {
        let {match, preData, saleInfo} = this.props;
        let billProdDataTags;
        if(match.params.id){
            billProdDataTags = saleInfo && saleInfo.getIn(['data', 'billProdDataTags']);
        }else{
    ...
    asyncSubmit=()=>{
        ...
        this.props.asyncAddSale(values.billNo, values, (res) => {
            if (res.data.retCode === '0') {
                ...
                let url = continueAddFlag ? '/blank' :`/sale/show/${displayId}`;
                this.props.history.push(url);
                if(continueAddFlag) this.props.history.replace('/sale/add');
    ...
    resetDefaultFields = ()=>{
        ...
        if(id){
            this.props.asyncFetchSaleById(id, ()=>{
                this.props.setInitFinished();
            })
        }else{
    ...
    render() {
        let currencyVipFlag = getCookie("currencyVipFlag");
        const {saleInfo, preData, match, goodsTableConfig, rateList} = this.props;
        ...
        if(match.params.id){
            ...
            billFields = saleInfo && saleInfo.getIn(['data', 'billProdDataTags']);
        ...
        if(id){
            ...
            if(saleInfoData && saleInfoData.get('billNo') === id) {
            ...
            <React.Fragment>
                <Content.ContentHd>
                    <Crumb data={getUrlParamValue('source') === 'mall'?[
                        {
                            url: '/mall/',
                            title: intl.get("sale.add.index.mall")
                        },
                        {
                            url: '/mall/sale/',
                            title: intl.get("sale.add.index.onlineSaleOrder")
                        },
                        {
                            title: this.props.match.params.id? intl.get("sale.add.index.modifySaleOrder"): intl.get("sale.add.index.addSaleOrder")
                        }
                    ]:[
                        {
                            url: '/sale/',
                            title: intl.get("sale.add.index.sale")
                        },
                        {
                            title: this.props.match.params.id? intl.get("sale.add.index.modifySaleOrder"): intl.get("sale.add.index.addSaleOrder")
                        }
                ...
                <Content.ContentBd>
                    <Spin
                        spinning={preData.get('isFetching') || saleInfo.get("isFetching")}
                    >
                        <AddForm {...this.props}
