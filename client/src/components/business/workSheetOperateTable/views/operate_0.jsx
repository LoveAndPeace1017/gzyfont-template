...
export default class Operate extends Component {
    ...
    handleOnBlur = (value, key, type) => {
        let quantityDecimalNum = Number(getCookie("quantityDecimalNum")||3);
        let argKey = (type === 'finishCount') ? 'scrapCount' : 'finishCount';
        let argValue = this.props.getFormField(key, argKey) || 0;
        let reportCount = _.add(_.toNumber(value), _.toNumber(argValue));
        reportCount = fixedDecimal(reportCount, quantityDecimalNum);
        this.props.setFormField(key, {reportCount});
    };
        ...
        let columns = [
            {
                ...
                render: (text, record, index)=> {
                    return (
                        ...
                        >
                            <Input maxLength={14} className={cx("readOnly")} readOnly/>
                        </Form.Item>
                    )
                }
            }, {
                ...
                render: (text, record, index)=> {
                    return (
                        <Form.Item
                            name={[dataPrefix, record.key, 'finishCount']}
                            rules={[
                                { required: true,message: "该项为必填项"},
                                {
                                    validator: (rules, value, callback) => {
                        ...
                        >
                            <Input maxLength={14} onBlur={(e) => this.handleOnBlur(e.target.value, record.key, 'finishCount')}/>
                        </Form.Item>
                    )
                }
            }, {
                ...
                render: (text, record, index)=> {
                    return (
                        <Form.Item
                            name={[dataPrefix, record.key, 'scrapCount']}
                            rules={[
                                { required: true,message: "该项为必填项"},
                                {
                                    validator: (rules, value, callback) => {
                        ...
                        >
                            <Input maxLength={14} onBlur={(e) => this.handleOnBlur(e.target.value, record.key, 'scrapCount')}/>
                        </Form.Item>
                    )
                }
            }
