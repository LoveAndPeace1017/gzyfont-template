...
export default class ProductList extends Component {
    ...
    componentDidMount() {
        ...
        let {moduleType, priceType, QUANTITY_MAP, batchShelfLeftFlag, currencyVipFlag, prodDataTags, billProdDataTags} = this.props;
        let columns = [
            {
        ...
        columns.push(
            {
                title: this.props.fieldConfigType === 'quotation'?'报价数量':QUANTITY_MAP[moduleType],
                key: 'recQuantity',
        ...
        columns.push({
            title: intl.get("components.productList.index.remarks"),
            dataIndex: 'remarks',
            key: 'remarks',
            columnName: 'remarks',
            width: 150,
        ...
        moduleType === 'purchase' && (
            columns.splice(-1, 0,
                {
                    title: '上游订单',
                    dataIndex: 'upstreamBillNo',
                    key: 'upstreamBillNo',
                    width: 200,
                    render: (text, row) => {
                        let displayBillNo = row.displaySaleBillNo || row.requestDisplayBillNo;
                        let billNo = row.saleBillNo || row.requestBillNo;
                        let url = row.saleBillNo ? `/sale/show/${billNo}` : `/purchase/requisitionOrder/show/${billNo}`;
                        return (
                            <React.Fragment>
                                <span className={cx("txt-clip")} title={displayBillNo}>
                                    <Link to={url}>{displayBillNo}</Link>
                                </span>
                            </React.Fragment>
                        )
                    }
                ...
                {
                    title: intl.get("components.productList.index.inQuantity"),
                    key: 'inQuantity',
                ...
                {
                    title: intl.get("components.productList.index.unStockIn"),
                    key: 'unStockIn',
                    dataIndex: 'unStockIn',
                    align: 'right',
                    width: 120,
        ...
        if(moduleType === 'sale' && this.props.fieldConfigType === 'quotation'){
            columns.splice(-1, 0,
                {
                    title: intl.get("components.productList.index.deliveryDeadlineDate"),
                    dataIndex: 'deliveryDeadlineDate',
        ...
        }else if(moduleType === 'sale' && this.props.fieldConfigType !== 'quotation'){
            columns.splice(-1, 0,
                {
                    title: intl.get("components.productList.index.deliveryDeadlineDate"),
                    dataIndex: 'deliveryDeadlineDate',
        ...
        (moduleType === 'purchase' || moduleType === 'sale') && (
            columns.push(
                {
                    title: intl.get("components.productList.index.operate"),
                    key: 'operate',
                    dataIndex: 'operate',
                    width: 100,
                    render: (value, item) => {
                        return value && (
                            <Button style={{fontSize: 11}} loading={item.isFetching}
                                    onClick={() => this.props.operateClick(item)}
                            >
                                {value}
                            </Button>)
                    }
        ...
        (moduleType === 'inbound' || moduleType === 'outbound') && ((
            columns.push(
                {
                    title: intl.get("components.productList.index.serialNumber"),
                    key: 'serialNumber',
                    dataIndex: 'serialNumber',
                    columnName: 'serialNumber',
                    width: 150,
                    render: (serialNums, item) => {
                        return serialNums && serialNums.length > 0 && (
                            <Dropdown
                                overlay={() => (
                                    <Menu className={cx('abstract-drop-menu')}>
                                        <Menu.Item>
                                            <div className={cx("abstract-drop")}>
                                                <div className={cx("tit")}>{intl.get("components.productList.index.serialNumber")}</div>
                                                <ul>
                                                    {
                                                        serialNums.map((serialNum, index) =>
                                                            <li key={index}>
                                                                <span className={cx('prod-tit')}>{serialNum}</span>
                                                            </li>
                                                        )
                                                    }
                                                </ul>
                                ...
                                <span>{serialNums[0]}<DownOutlined className="ml5"/></span>
                            </Dropdown>
                        )
                    }
                }
