...
/**
 *
 * @visibleName RequisitionTable（请购单列表）
 */
@withCustomFields
@withFix
@withFormOperate
@withGoodsPop
@withGoodsSuggest
export default class RequisitionTable extends Base {
    constructor(props) {
        super(props);
    }
    ...
    render() {
        ...
        let columns = [
            ...
            ...renderProdCustomFields(prodCustomFields),
            {
                ...
                render: (text, record)=> {
                    return (
                        ...
                        >
                            <Input maxLength={14} onBlur={()=>this.props.calculateTotalQuantity(record.key,'quantity')}/>
                        </Form.Item>
                    )
                }
            },
            {
                ...
                render: (text, record)=> {
                    return (
                        ...
                        >
                            <Input maxLength={14} onBlur={()=>this.props.calculateTotalQuantity(record.key,'unitPrice')}/>
                        </Form.Item>
                    )
                }
            },
            {
                ...
                render: (text, record)=> {
                    return (
                        ...
                        >
                            <Input maxLength={14} onBlur={()=>this.props.calculateTotalQuantity(record.key,'amount')}/>
                        </Form.Item>
                    )
                }
            },
            ...
            {
                title: <div>期望交付日期
                   <a href="#!" className={"fr"} onClick={this.props.oneKeyFill}>一键填充</a>
                </div>,
                            ...
                            rules={[
                                {
                                    validator: (rules, value, callback) => {
                                        ...
                                        if (requestDate && value && requestDate.isAfter(value, 'date')) {
                                            callback(`不能早于请购日期`);
                                        }
                                        callback();
                                    }
        ...
        footerStr = (
            <div className={cx("tb-footer-wrap") + " cf"}>
                <div className={cx('total')}>
                    ...
                    <span>预计请购总金额: <b><React.Fragment>
                                <Form.Item name={dataPrefix+"TotalAmount"}
                                           {...formItemLayout}
                                           initialValue={formatCurrency(0, 3)}
                                           className={'form-x'}
                                           style={{display: 'inline-block'}}
                                >
                                    <Input style={{color:"#e53e3e",width: "120px",position: "relative",top: "2px"}} bordered={false} disabled/>
                                </Form.Item>
                    </React.Fragment></b></span>
                </div>
            </div>
