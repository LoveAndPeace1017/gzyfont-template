            ...
            if(this.props.showBatchLeftColumns){
                batchLeftColumns = [{
                    ...
                    rules: [
                        ...
                        //         let productCode = getFieldValue([this.dataPrefix, idx, this.dataName.productCode]);
                        //         let expirationFlag = getFieldValue([this.dataPrefix, idx, this.dataName.expirationFlag]);
                        //         if(this.props.source!=='wareOut' || !expirationFlag) return;
                        //         var flag = false;
                        //         flag = await new Promise((resolve, reject) => {
                        //             this.props.asyncFetchBatchShelfList({productCode, batchno: value}, (res)=> {
                        //                 if(res.data && res.data.retCode==="0") {
                        //                     let dataSource = res.data.data;
                        //                     if (dataSource && dataSource.length > 0) {
                        //                         resolve(true);
                        ...
                        //         });
                        //         if(!flag) {
                        //             return Promise.reject(new Error('批次不存在'));
                        //         }
                        //     },
                        // },
                        {
