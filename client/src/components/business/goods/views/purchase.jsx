...
import withDeadlineDate from '../dependencies/withDeadlineDate';
...
export default class Purchase extends Base {
    ...
    render() {
        let { dataPrefix, formData, goodsTableConfig, defaultForm, renderFixIcon, fixed=false, // 是否固定前几列
            specColumn, unitColumn, quantityColumns, deadlineDateColumn, footer, prodFields, billFields, renderCustomFields, renderProdCustomFields} = this.props;
        //处理字段配置
        ...
        let columns = [
                        ...
                        <GoodsSuggest
                            ...
                            suffixIcon={
                                <div
                                    ...
                                    <a onClick={() => this.props.selectGoods(index, record.key)}><EllipsisOutlined style={{fontSize: "16px"}}/></a>
                                </div>
                            }
                        />
                    </Form.Item>
            ...
            ...itemProdCustomFields,
            {
                ...
                key: 'upstreamBillNo',
                columnName: 'upstreamBillNo',
                width: 170,
                render: (text, record)=> {
                    let displayBillNo = record.displaySaleBillNo || record.requestDisplayBillNo;
                    return (
                        <React.Fragment>
                            <span className="txt-clip" title={displayBillNo}>{displayBillNo}</span>
                        </React.Fragment>
                    )
                }
            },
